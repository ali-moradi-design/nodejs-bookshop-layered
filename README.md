...
CORS is `credentials: true`. Set `CORS_ORIGIN` to the frontend origin(s), comma-separated. Default when unset: `http://localhost:5173,http://127.0.0.1:5173,http://localhost:3000,http://127.0.0.1:3000` (Vite + Next local). Do **not** use `*` in production with credentials.
